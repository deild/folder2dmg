...
fn make_github_issue_link(folder2dmg_version: &str, environment: Environment) -> String {
...
",
  ...
  format!(
    "https://github.com/deild/folder2dmg/issues/new?template={}&body={}",
...
mod tests {
  ...
  fn test_make_github_link() {
    let folder2dmg_version = crate_version!();
    let environment = Environment {
    ...
    let link = make_github_issue_link(folder2dmg_version, environment);

    assert!(link.contains(folder2dmg_version));
